...
def main():
    ...
    for fname in image_files:
        ...
        gt_path = os.path.join(GROUND_TRUTH_DIR, txt_name)

        if USE_GROUND_TRUTH_FALLBACK and os.path.exists(gt_path):
            shutil.copyfile(gt_path, txt_path)
            print(f"{fname}: using benchmark ground truth -> {txt_name}")
            continue
